...
written **`S`** (the thumb that types Space in Dosh) and **`B`** (the one that types
...
The mapping keeps it exactly, with **`S` as the word-end marker** — and since `S` is
Dosh's Space thumb, the mnemonic is exact: the space thumb adds the space.
...
VOWEL GROUP, right hand -- Series 3
  Seven identities; add S to end the word.
